document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- Logique pour la section ACTUALITÉS & ARCHIVES ---
    ...
    // --- Logique de tri et d'archivage ---
    const today = new Date();
    // Calcule la date limite : aujourd'hui moins 6 mois
    const cutoffDate = new Date(new Date().setMonth(today.getMonth() - 6));

    const sortedNewsIds = Object.keys(newsData).sort((a, b) => parseFrenchDate(newsData[b].date) - parseFrenchDate(newsData[a].date));
    
    // Filtre les événements : courants (moins de 6 mois)
    const currentNewsIds = sortedNewsIds.filter(id => parseFrenchDate(newsData[id].date) >= cutoffDate);
    // Filtre les événements : archivés (plus de 6 mois)
    const archivedNewsIds = sortedNewsIds.filter(id => parseFrenchDate(newsData[id].date) < cutoffDate);
    ...
    if (newsSliderWrapper) {
        ...
        if (typeof Swiper !== 'undefined') new Swiper('.news-slider', { 
            loop: currentNewsIds.length > 2, // Utilise la liste filtrée
            spaceBetween: 30, 
            slidesPerView: 1, 
    ...
    // --- Logique pour la page des archives (archive.html) ---
    ...
    if (archiveGridContainer) {
        if (archivedNewsIds.length > 0) {
            archivedNewsIds.forEach(id => {
                const newsItem = newsData[id];
                const cardDescription = newsItem.description.replace(/<[^>]*>/g, ' ').substring(0, 100).trim() + '...';
                
                // Crée la carte (similaire au slider, mais sans 'swiper-slide')
                const cardHTML = `
                <div class="bg-white rounded-lg shadow-lg overflow-hidden flex flex-col h-full transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 group">
                    ...
                    <div class="p-6 flex-grow flex flex-col">
                        ...
                        <button data-modal-id="${id}" class="modal-trigger mt-auto font-semibold text-primary inline-flex items-center self-start">
                            Lire la suite
                            <i data-lucide="arrow-right" class="ml-1 h-4 w-4 transition-transform duration-300 group-hover:translate-x-1"></i>
                        </button>
                    </div>
                </div>`;
                archiveGridContainer.innerHTML += cardHTML;
            });
        } else {
            // Message si aucune archive n'est trouvée
            archiveGridContainer.innerHTML = `<p class="text-center text-stone-600 col-span-full">Il n'y a pas d'événements archivés pour le moment.</p>`;
        }
    }

    // --- Logique MODALE (partagée par index.html et archive.html) ---
    ...
    document.querySelectorAll('.modal-trigger').forEach(button => { 
        button.addEventListener('click', () => { 
            ...
            if (data) { 
                ...
                let contentHTML = `<p class="text-stone-700">${data.full_description || data.description}</p>`; 
